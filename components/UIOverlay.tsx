...
export const UIOverlay: React.FC<UIOverlayProps> = ({ mode, onToggle, onPhotosUpload, hasPhotos, uploadedPhotos, isSharedView }) => {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (urlsResponse.status === 404) {
        ...
        if (isLocalDev) {
          ...
          try {
            ...
            setShareError('Image data too large, please reduce the number or size of photos');
            return;
          }
        } else {
      ...
      if (!completeResponse.ok) {
        throw new Error(completeData.error || 'Failed to save share info');
      }
  ...
  const handleCreateMine = () => {
    // Clear URL params and refresh page
          ...
          <>
            ...
            {!hasPhotos && (
              <button
                ...
                className="group px-6 py-3 sm:px-4 sm:py-2 border-2 border-[#D4AF37] bg-black/70 backdrop-blur-md overflow-hidden transition-all duration-500 hover:shadow-[0_0_30px_#D4AF37] hover:border-[#fff] hover:bg-[#D4AF37]/20 min-h-[44px] min-w-[44px]"
              >
                  <span className="relative z-10 font-serif text-base md:text-lg sm:text-sm text-[#D4AF37] tracking-[0.1em] group-hover:text-white transition-colors whitespace-nowrap">
                  Upload Photos
                </span>
              </button>
            )}
            ...
            {shareLink && (
              <div className="bg-black/80 backdrop-blur-md border-2 border-[#D4AF37] p-4 max-w-sm">
                <p className="text-[#F5E6BF] font-serif text-sm mb-2">Share link created</p>
